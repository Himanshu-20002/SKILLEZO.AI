    ...
    <AuthCard className="max-w-md w-full">
      ...
      <div className="min-h-[88px] sm:min-h-[92px] flex flex-col justify-start">
        <AuthHeader
          ...
          title={activeRole === "recruiter" ? "Recruiter Portal 🏢" : "Welcome Back 👋"}
          subtitle={
            activeRole === "recruiter"
              ? "Sign in to review verified talent pipeline and stream candidate resumes."
              : "Sign in to access your skills dashboard and verified career credentials."
          }
        />
      </div>
      ...
      <div className="pt-1 text-center text-xs sm:text-sm text-[#8A90A6]">
        ...
        >
          Sign Up as {activeRole === "recruiter" ? "Employer" : "Candidate"}
        </Link>
      </div>
    </AuthCard>
  );
